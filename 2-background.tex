...
The centralized scheduler is a high-performance Delegation Ticket Lock (DTLock) based scheduler~\cite{alvarezAdvancedSynchronizationTechniques2021a}\cite{alvarez2022nosv}, on which only one thread has access to the scheduler data structures at a time, and is responsible for not only finding a task for itself, but for scheduling the other hungry threads as well. 
...
Task submission to nOS-V allows multiple tasks to be submitted concurrently from different threads without needing to aquire the scheduler lock. To that end, each thread has a lock-free Single-Producer Single-Consumer queues (SPSC) that are later consumed by the thread running the scheduler code. 
